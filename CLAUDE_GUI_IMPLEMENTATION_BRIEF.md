...
일부 회차만 다운로드했어도 전체 state·metadata 목록으로 형제 문맥을 판단한다. 구형
순번 폴더에 ID를 복구할 때는 제목의 유일한 일치가 필요하며 번호만으로 연결하지 않는다.
이미지 미리보기는 source ID 또는 정확한 폴더명으로 선택하며 같은 순번의 다른 회차
이미지를 합치지 않는다. CLI의 `preview --episode-id`·`--episode-folder`로도 검증한다.
