...
class LibraryCliIntegrationTests(unittest.TestCase):
    ...
    def test_actual_cli_deletes_records_and_refreshes_real_gui_without_deleting_files(self):
        first, first_root, first_episode = self.work("a")
        second, second_root, second_episode = self.work("b")
        ...
        with (self.root / "host.log").open("w", encoding="utf-8") as log:
            ...
            try:
                ...
                self.cli("library", "select", "--job", first.job_id, "--job", second.job_id, "--json")
                args = ("library", "delete", "--job", first.job_id, "--job", second.job_id, "--kind", "records")
                ...
                self.assertEqual(result["result"]["removedRecordCount"], 2)
                ...
                for root, episode in ((first_root, first_episode), (second_root, second_episode)):
                    self.assertEqual(len(list(episode.glob("*.jpg"))), 3)
                    self.assertTrue((root / "metadata.json").is_file())
                print("[CLI integration] removed 2 records; database 0; GUI 0 after refresh; original images 6 preserved")
            finally:
