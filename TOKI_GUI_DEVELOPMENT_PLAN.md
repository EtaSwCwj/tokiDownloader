...
- 2026-09-11 이미지 대응표 이어받기: ZIP 재검사는 제외한다. 회차 단위 원본 URL 목록·파일
  대응표와 bounded 안정화 검사로 중단/삽입/순서 변경을 처리한다. 변경 없는 대응표는 추가
  복사·다운로드 없이 검증하며, GUI 목록 로딩에는 이미지 I/O를 추가하지 않는다.
  회귀 검증은 실제 프로세스 중단, 복구, 숨김 Chromium 지연 로딩 사례를 포함한다.
- 2026-09-10 감상 순서 공통화 완료: 사이트 `number`/`sourceId`와 별도로 `readingOrder`를 계산한다.
  본편·독립 외전을 분리하며 소수/분할 및 본편 귀속 외전은 본편 위치를 유지한다.
  번호 없음/범위/부·시즌/혼합 단위의 모호한 항목은 판단 보류 사유와 기존 상대 위치를 보존한다.
  Node 신규 저장, Python 폴더명 정리, 작품 전체 ZIP이 공유 정책과 교차 런타임 테스트로 일치한다.
  폴더명 정리는 백업·트랜잭션을 사용하고 ZIP 내부 갱신은 기존 전체 압축 서비스로 수행한다.
  기존 파일을 목록 표시 중 자동 이관하지 않는다. 정렬 계산은 메타데이터만 읽는 O(n log n)이며
  1만 회차 합성 데이터로 검증한다.
...
- 2026-09-08: 전체 목록 삭제 후 첫 enqueue의 카운터를 먼저 갱신해 빈 상태 패널이 남지
  않도록 수정하고 실제 CLI 프로세스로 재검증했다. 연결된 GUI의 짧은 ping 응답 지연은
  종료로 오인하지 않는다. CDN 연결 reset은 별도 실접속 검증과 UI 검증을 구분해서 보고한다.
- 2026-09-08 후속: 직접 HTTPS 이미지의 인증 전 `ECONNRESET`에만 TLS ClientHello
  레코드 분할 재시도를 적용한다. 인증서 검증/프록시 경로/동시성/속도 제한을 유지하고
  성공 호스트 캐시는 실행별 128개로 제한한다. 로컬 TLS reset 재현과 실제 실패 회차의
  다운로드 성공을 함께 검증한다. 시스템 DNS·VPN·보안 설정을 자동으로 변경하지 않는다.
...
사용자 피드백으로 선택 드롭다운을 2×2 작업 버튼으로 바꿨다. 버튼을 누르면 대상
검사 → 확인창 → 승인 시 실제 처리까지 이어진다. 기록 삭제 성공 시 창을 닫고 목록을
즉시 갱신한다. 독립 프로세스 CLI와 실제 MainWindow/IPC/DB를 연결하는 통합 검사에서
임시 기록 삭제, 새로고침 후 빈 목록과 원본 파일 보존을 검증한다.
실제 작품과 모의 실행 기록이 섞인 선택에서도 파일 처리가 막히지 않도록 모의 기록만
명시적으로 제외한다. 파일 대상이 없으면 실행·확인창·휴지통 생성 없이 결과를 안내한다.
모의 기록의 목록 삭제는 지원하며 실제 경로의 공유·상위 폴더 보호는 완화하지 않는다.
