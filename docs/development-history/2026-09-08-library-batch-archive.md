...
## 같은 날 후속 UI 수정: 드롭다운 대신 작업 버튼

기준 커밋 `9c8cdfd`. 사용자가 Delete 선택창의 드롭다운이 불편하다고 요청해 네 작업을
2×2 버튼으로 모두 노출했다. 버튼에는 처리 범위·보존 대상을 표시하고, 선택된 버튼을
강조한다. 버튼 한 번으로 대상 미리보기까지 진행하되 실제 변경에는 기존 최종 확인이
필요하다. 작업 중에는 종류 변경을 막으며 선택을 바꾸면 이전 미리보기를 무효화한다.
압축 창의 한 항목뿐인 드롭다운도 제거하고 고정 안내로 표시한다.

서비스/삭제 범위/CLI 명령은 변경하지 않았다. `--show-gui --kind`는 해당 버튼을 선택하고
`--dry-run`은 같은 미리보기로 이어진다. 모든 버튼의 read-only 미리보기와 취소, CLI
...
## 같은 날 후속 수정: 모의 기록이 섞인 파일 삭제

기준 커밋 `055c49f`. 09:05 캡처는 목록 삭제가 아닌 압축 파일 삭제였다. 실제 작품과
공유 출력 폴더를 쓰는 YouTube 모의 기록 3개를 함께 선택하면 `_safe_root` 보호 예외가
전체 batch를 중단시켰다. 보호를 해제하지 않고 `simulation=True` 기록만 파일/압축 삭제
계획에서 제외하도록 했다. 제외 이유와 개수, 처리 가능한 작품 수를 결과와 화면에 표시한다.
실제 작업의 공유·상위 폴더와 링크 보호는 그대로다. 목록 삭제는 모의 기록도 처리한다.

`canExecute`, `eligibleJobCount`, `skippedJobs`, `skippedJobCount`, `emptyJobCount`와
`movedFileCount`를 CLI JSON에 추가했다. 계획 토큰은 제외 ID도 포함한다. 삭제할 파일이
없는 요청은 `noOp: true`, `executed: false`이며 휴지통/manifest 생성, 상태 변경과 확인창이
없다. GUI의 실행 버튼도 비활성화한다. `파일 삭제`와 `압축 파일 삭제` 모두 같은 정책이다.

서비스/GUI 테스트에 모의 공유 폴더 보호와 대상 없음 안내를 추가했다. 실제 CLI 통합 검사도
**실제 작품 1개 + 모의 3개**로 확장했다. ZIP 1개 이동 → 빈 대상 재시도 무변경 → ZIP 복구
→ 원본 이미지 3개 이동 → 복구 → 기록 4개 삭제 → DB/GUI/새로고침 후 0개를 확인했다.
각 단계에서 모의 작업 공유 폴더의 보호 파일은 유지되며 기록 삭제 후 원본·ZIP도 보존한다.
현재 사용자 데이터는 CLI 미리보기와 화면 확인에만 사용하고 실제 파일·기록 삭제는 하지 않는다.

최종 Python 전체 **393건**(실제 CLI 프로세스 통합 검사 포함), Node 전체 **58건**이 통과했다. 실행 중인 사용자
GUI도 수정 버전으로 안전하게 다시 열었다. 같은 4개 기록에 실제 CLI `--dry-run`을 호출해
파일 삭제는 대상 **2,242개 / 모의 기록 제외 3개**, 압축 삭제는 **대상 0개 / 제외 3개 /
파일 없는 실제 작품 1개**임을 확인했다. 원본 기록 4개는 보존했다.
`logs/2026-09-08-library-mixed-selection.png`를 직접 캡처해 대상 없음 안내, 제외 사유와
실행 버튼 비활성화 상태를 확인했다. 구문 검사와 `git diff --check`도 통과했다.
