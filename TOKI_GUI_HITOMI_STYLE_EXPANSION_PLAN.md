...
2026-08-07 대량 목록 표시 정책 구현: 설정 스키마 v11에 페이지 크기 25~1,000개,
메모리 내 작품 상한 100~5,000개, 스크롤 1~20단계, 지연 로딩과 저사양 모드를 추가했다.
공용 정책 서비스와 `list-performance status|set`, GUI 고급 설정 및 실행 중 목록 재로딩이
같은 값을 사용한다. 기본은 SQLite 200개 페이지·최대 2,000개·3단계·지연 로딩이며 화면에
보이는 썸네일만 디코딩한다. 저사양 모드는 사용자가 저장한 값을 지우지 않고 유효값만
100개 페이지·500개 상한·3단계·지연 로딩·썸네일 숨김·메모리 캐시 32개로 제한한다.
...
2026-08-07 회차별 PDF 생성 구현: 설정 스키마 v13에 기본 꺼짐 자동 생성 선택을 추가하고,
공용 서비스가 작품의 회차 폴더를 자연 숫자 순서로 읽어 `_pdf/<회차 폴더명>.pdf` 계획과
실행 결과를 만든다. Pillow로 각 PDF를 임시 파일에 완성한 뒤 원자 교체하며 원본 이미지와
메타데이터는 변경하거나 삭제하지 않는다. 원본이 그대로인 최신 PDF는 건너뛰고 변경된
회차의 생성 PDF만 교체하며, 중지·실패 뒤 남은 임시 파일은 다음 실행에서 복구한다.
`pdf status|set|plan|generate|cancel|close`, GUI 고급 설정, 작품 우클릭 계획/진행 창과 자동
후처리가 같은 서비스를 사용한다. 실제 생성은 `--execute --yes` 또는 GUI 재확인이 필요하고
자동 후처리는 이미지 변환과 공유하는 제한형 CPU 프로세스 슬롯이 찼을 때 대기열에 남아
완료 후 동작보다 먼저 처리된다. 임시 한글·공백 작품에서 2개 회차·3개 이미지의 실제 PDF,
PDF 서명, 원본 바이트 불변, 재실행 건너뛰기, 원본 변경 뒤 선택 교체, 취소와 임시 파일
...
Python 263건, Node 16건, GUI 자체 점검 6/6과 필수 환경 5/5를 통과했다. 최종 안전 감사에서
쿠키·갤러리 토큰이 다른 호스트로 전달되지 않도록 리다이렉트를 동일 스킴·호스트·포트로
...
제한한다. Windows 금지 문자·경로 구분자·예약 장치 이름을 정리하고 대소문자 중복에는
카운터를 붙여 덮어쓰기를 막는다. E-Hentai 요약처럼 원본 이름이 없으면 숫자 방식은
`.jpg` 계획을 만들고 원본 기반 방식은 `hitomi.filename_original_missing`으로 명확히
거부한다. `hitomi filenames status|set|plan`, GUI 공급자 탭, `status --json`이 같은 정책을
사용하며 실행 중 GUI에서 원본 방식 적용과 기본 숫자+원본 복원을 확인했다.
`logs/hitomi-filename-settings.png`에서 한글·대비·기본 선택을 검토했고 외부 요청이나 파일
생성은 없었다. 100,000장 숫자 방식도 반환 샘플 3개로 제한하는 회귀 테스트를 포함해
Python 219건, Node 16건, 자체 점검 6/6을 통과했다.

2026-08-07 제외 태그 관리 구현: 설정 스키마 v19에 최대 500개, 규칙당 100자 제한의
정규화된 제외 태그를 추가했다. 네임스페이스가 있는 규칙은 정확히 일치하고 이름만 있는
규칙은 `female:`·`male:` 등 어느 네임스페이스의 같은 이름에도 일치하며, 결과는 일치 규칙과
실제 태그를 함께 반환한다. 공용 서비스와 `hitomi tags status|set|evaluate`, GUI 공급자 탭의
여러 줄 편집기, `status --json`이 같은 정책을 사용한다. 로컬 Hitomi 픽스처에서 `full color`
규칙이 `female:full color`를 제외하는지 확인하고 실행 중 GUI에 CLI로 두 규칙을 적용한 뒤
...
2026-08-07 인증 공급자 사용자 쿠키 처리 구현: 기존 Windows 자격 증명 저장소 관리 범위를
Hitomi.la·ExHentai·E-Hentai로 넓히고, 비밀값을 읽지 않는 `cookies policy`에서 인증 필요,
허용 도메인과 알려진 쿠키 이름 힌트만 확인하게 했다. 사이트가 로그인 성공에 필요한 전체
쿠키 조합을 고정 규격으로 공개하지 않으므로 특정 이름만으로 준비 완료를 단정하지 않는다.
`plan-import`도 민감 파일을
읽기 전에 `--yes`를 요구하며, ExHentai 계열은 관련 도메인의 쿠키만 보관하고 다른 사이트
쿠키는 제외한다. 실제 요청은 기본적으로 쿠키를 전혀 읽지 않고 CLI
`hitomi metadata fetch --use-cookies --yes` 또는 GUI 체크·재확인에서만 OS 저장소를 읽는다.
요청 호스트·경로·Secure·만료 조건을 만족하는 값만 메모리의 Cookie 헤더에 넣고 결과·상태·
로그에는 값을 반환하지 않는다. 헤더 삽입 공격 문자, 공급자와 다른 도메인, 관련 쿠키 없음도
명시적으로 거부하며 접근 제한 우회는 지원하지 않는다. 메모리 보안 저장소와 임시 가짜
쿠키만 사용해 저장·필터·요청 주입·비노출을 검증했고 실제 사용자 쿠키나 외부 API는 읽거나
호출하지 않았다. 화면은 `logs/hitomi-auth-cookie-manager.png`와
`logs/hitomi-auth-cookie-fetch-option.png`에 보존했다.
Python 233건, Node 16건, 자체 점검 6/6과 필수 환경 5/5를 통과했다.
...
2026-08-07 YouTube 썸네일·메타데이터 정책 구현: 설정 스키마 v26에 기본 꺼짐인 대표
썸네일 파일 저장, 표지 포함, 정리된 `.info.json`, `.description`, 미디어 태그 포함 여부를
추가했다. `youtube metadata status|set|plan`과 공급자 설정의 다섯 체크박스가 같은 공용
정책을 사용한다. 정보 JSON은 개인 정보가 포함될 수 있음을 화면과 상태에 표시하고,
`--clean-info-json`, `--no-write-comments`, `--no-write-playlist-metafiles`을 명시한다. 별도 댓글
수집은 요청하지 않지만 추출기가 즉시 제공하는 댓글은 남을 수 있음을 숨기지 않는다.
미디어 태그 포함은 다음 단계의 챕터와 infojson 첨부를 암묵적으로 켜지 않도록
`--no-embed-chapters`, `--no-embed-info-json`을 함께 계획한다. 실행 중 GUI에 다섯 옵션을
CLI로 적용해 상태 JSON과 화면을 확인한 뒤 모두 기본값으로 복원했다.
...
2026-08-07 YouTube 업로드 날짜 파일 시간 구현: 설정 스키마 v29에 기본 꺼짐 정책을
추가하고 yt-dlp의 HTTP Last-Modified 기반 `--mtime`과 혼동하지 않도록 별도 완료 후 서비스로
구현했다. `upload_date`를 엄격한 YYYYMMDD로 검사해 UTC 자정 타임스탬프로 변환하고 파일
수정 시각만 바꾸며 접근 시각은 보존한다. 심볼릭 링크와 없는 파일을 거부하고 실제 적용은
`--yes`가 필요하다. `youtube mtime status|set|plan|apply`, GUI 체크박스, 형식 계획의
`postDownloadActions`가 같은 공용 서비스를 사용한다. 임시 한글 파일에서 미확인 적용 거부,
확인 적용, 정확한 나노초 수정 시각과 접근 시각 보존을 검증했다. 실행 중 GUI에 CLI로 설정을
켜고 존재하지 않는 예시 경로를 계획해 UTC/로컬 시각과 미실행 상태를 화면에서 확인한 뒤
기본 꺼짐으로 복원했다. `logs/youtube-upload-date-mtime.png`에 화면을 보존했으며 사용자
