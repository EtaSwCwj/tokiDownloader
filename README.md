...
이미 등록된 작품이면 **목록 한 줄을 유지하며 회차 목록을 다시 수집하고 새 회차만 받습니다.**
완료·오류·중지·취소된 작업이 대상이며, 실행·대기·재시도 대기·일시정지 중이면 중복 실행하지
않습니다. 화면 필터나 로딩 여부와 무관하게 전체 DB에서 작품을 찾습니다. 기존 작품은 저장
폴더·메모·고정 표시·메타데이터를 유지하고 실행 이력만 추가합니다. 같은 주소를 나중에 다시
복사해도 갱신됩니다. 이미 받은 회차와 ZIP에 보존된 회차는 기존 완료 판정에 따라 건너뜁니다.

새 작품은 **저장된 기본 폴더와 다운로드 설정**을 사용합니다. 기존 작품은 기본 폴더가 바뀌어도
해당 작품의 저장 폴더를 계속 사용합니다. 두 경우 모두 입력창의 URL·회차 범위를 바꾸거나
상속하지 않고, 회차 제한 없이 신규 회차 검사를 수행합니다. 다운로드 후 자동 ZIP/원본 정리도
기존 설정을 따릅니다. 감지 실패는 로그와 `clipboard status`의 `lastInspection`에서 확인할 수
있으며 무관한 클립보드 텍스트는 로그에 남기지 않습니다. `refreshed=true`와 `refresh_queued`는
기존 작품 갱신 예약, `already_active`는 이미 진행 중이라 추가하지 않았다는 뜻입니다.
...
사이트 목록의 제목 자체가 비어 있는 행은 `unsafe_episode_title` 사이트 구조 오류로
중단하며 둘 다 자동 재시도하지 않습니다. 제목은 있지만 화수·부제가 없는 회차는
건너뛰지 않고 사이트 순번으로 저장합니다. 예: `000011 원펀맨 리메이크`.
임의의 `11화`를 붙이지 않으며 원래 제목과 고유 회차 URL을 메타데이터에 보존합니다.
긴 전체 경로는 제목을 임의로 자르지 않고 더 짧은 `-output` 경로를 안내합니다.
진행 기록과 이미지 파일 교체 중 `EPERM`·`EACCES`·`EBUSY`가 발생하면 최대 9번,
총 5.5초의 대기를 두고 재시도합니다. 잠금이 계속되면 오류로 끝내되 기존 파일을
지우지 않으며, 진행 기록의 새 임시 파일도 복구용으로 남깁니다.
정상 JPEG 뒤에 부가 데이터가 붙은 경우에는 실제 디코딩 검사에 성공해야 허용하며,
받은 이미지 바이트를 잘라내거나 재인코딩하지 않습니다.
사이트 본문이 `이미지 처리 중인 회차입니다. 잠시 후 다시 확인해주세요.`라고 명시한
회차는 `.toki-state.json`의 `pendingEpisodes`에 미완료로 기록하고 다음 회차를 받습니다.
준비 중인 회차를 완료 ID에 추가하지 않으므로, 나중에 신규 회차 검사로 다시 받을 수 있습니다.
미수신이 남아도 받을 수 있는 회차 처리가 끝나면 `완료 · 미수신 N개`로 정상 종료합니다.
진행률 100%는 작업 처리 완료를 뜻하며 모든 회차를 보유했다는 뜻은 아닙니다.
기존 설정대로 받은 회차를 작품 전체 ZIP으로 압축하고, 검증 후 원본 정리도 적용합니다.
ZIP에는 `metadata.json`과 `.toki-state.json`을 넣으며 두 파일은 작품 폴더에도 보존합니다.
미수신 회차를 나중에 받으면 기존 ZIP의 다른 회차를 유지하면서 순서에 맞게 추가합니다.
CLI `info`, `list`, `runs`, `run-info`에서도 미수신 수를 확인할 수 있고 JSON에는
`pending_episode_count`, `completion_note`가 별도로 저장됩니다.
일반 빈 페이지·로그인 요구·인증 차단에는 이 건너뛰기 정책을 적용하지 않습니다.
알려진 이미지 형식의 응답이 3회 모두 검증에 실패한 경우에도 해당 회차를 미수신으로
기록하고 다음 회차를 진행합니다. 확장자/실제 형식 불일치, JPEG/PNG 끝 표식 누락,
JPEG 해독 실패, WebP 컨테이너 손상이 대상입니다. 실패 이미지의 URL·파일명·사유·
검출 형식·재시도 횟수는 `pendingEpisodes[].failedImages`에 남깁니다.
해당 회차에서 이미 받은 정상 이미지는 보존하되 불완전한 회차를 ZIP에 넣거나 정리하지 않습니다.
다음 검사에서는 정상 이미지를 재사용하고 빠진 이미지만 받습니다.
HTML/빈 응답·통신 오류·인증 오류·저장 오류는 미수신 완료로 바꾸지 않습니다.
한 회차에서 손상 이미지와 이런 오류가 함께 발생해도 전체 오류로 보고합니다.
...
2026-09-10부터 이 순번은 사이트 게시물 순번과 별도의 **감상 순서**입니다.
프로그램 공통 규칙으로 본편을 숫자순으로 정렬한 뒤 독립 외전·번외편·특별편·스핀오프·후일담을
각 그룹의 숫자순으로 배치합니다. `141화 외전`은 본편 141화 바로 뒤에 두고,
`141.2화`, `140-2화` 같은 소수·분할 회차는 본편 위치를 유지합니다.
번호 없는 외전, 범위 표기, 부/시즌 표기처럼 해석이 모호한 항목은 기존 상대 위치를 기준점으로
남기며, `readingOrderWarnings`와 GUI/CLI 안내에 사유를 표시합니다. 권/화 단위를 섞어 정렬하지 않습니다.
`number`와 고유 `sourceId` 및 완료 ID는 변경하지 않고 `readingOrder`, `readingGroup`,
`readingOrderVersion`, `readingOrderWarning`을 별도로 기록합니다. 범위 다운로드는 여전히 사이트 순번 기준입니다.
신규 폴더와 작품 전체 ZIP은 같은 규칙을 사용합니다. 기존 폴더는 아래 정리 명령으로 적용하고,
기존 ZIP은 작품 전체 압축을 다시 실행하면 원본 재다운로드 없이 내부 감상 순서가 갱신됩니다.
이미 압축 후 원본을 정리했어도 기존 ZIP의 이미지 바이트를 보존하면서 갱신합니다.
원본 폴더 이름 정리만 실행하는 경우 ZIP 내부는 즉시 재작성하지 않습니다.

```powershell
.\toki-cli.cmd rename-episodes --job 작품ID --dry-run --json
.\toki-cli.cmd rename-episodes --job 작품ID --execute --yes --json
.\toki-cli.cmd library archive --job 작품ID --execute --yes --json
```
...
오판하지 않습니다.
전체 재검사에서 ZIP에 들어간 회차는 기존처럼 건너뜁니다. 나머지 회차의 기존 이미지는
원본 URL 대응표가 일치하고 확장자·signature와 JPEG/PNG 종단 표식이 유효할 때만
재사용합니다. WebP는 RIFF 선언 길이, 각 청크의 경계·패딩, 이미지 청크와 애니메이션 프레임
...
#### 중단 후 이어받기와 회차 이미지 목록 검증 (2026-09-11)

- 이미지 목록은 숨겨진 본문 이미지와 지연 로딩 주소도 포함합니다. 문서 로딩 완료 후
  순서·개수·본문 높이가 1.25초 동안 안정돼야 확정하며, 최대 15초 안에 확정되지 않으면
  재시도 가능한 오류로 남깁니다. 지원하는 명시 개수 속성(`data-image-count`,
  `data-total-images`)이 있으면 실제 추출 개수와 대조합니다.
- 주소 없는 이미지, 잘못된 주소, 이전 기록보다 짧은 목록은 조용히 무시하지 않습니다.
  다운로드 직후에도 목록을 다시 읽어 추가·순서 변경이 있으면 완료로 기록하지 않습니다.
- `작품 폴더/.toki-image-resume/<회차 ID 해시>/manifest.json`에 원본 URL과 파일의
  대응표를 먼저 저장합니다. 번호만 같은 파일은 재사용하지 않으며, URL의 쿼리도 보존합니다.
  같은 URL이 목록에 반복되면 페이지 개수와 위치를 그대로 유지합니다.
- 대응표가 없는 기존 회차는 첫 재검사에서 해당 회차의 이미지를 다시 받습니다.
  ZIP 회차에는 적용하지 않습니다. URL만 바뀌었거나 기존 파일의 크기·수정 시각이 달라져
  동일 원본임을 확인할 수 없으면 재다운로드하는 보수적인 방식입니다.
- 새 이미지는 회차별 임시 저장소에 원자 저장합니다. 중간 종료 시 다음 실행에서 정상
  임시 이미지를 재사용하며, 전부 확보한 뒤 `0000.ext` 순서로 최종 배치합니다.
  변경된 기존 파일은 삭제하지 않고 `.toki-trash/image-resume/`에 복구 가능하게 보존합니다.
  이 복구 폴더와 임시 저장소는 회차 ZIP의 입력에 포함되지 않습니다.
- 변경 없는 완료 대응표는 파일 복사·다운로드 없이 검사만 합니다. 새 다운로드·재배치 중에는
  임시 사본 공간과 추가 디스크 I/O가 필요하며, 최종 배치 후 관리용 임시 이미지만 정리합니다.
  복구 보관된 이전 이미지는 자동 삭제하지 않습니다.
- 이 검사는 사이트가 실제 원본 목록을 빠뜨린 경우나 같은 URL의 내용만 바꾼 경우를 완전히
  판별하지는 못합니다. 사이트에서 실제 이미지가 삭제돼 목록이 줄었어도 자동 수용하지 않고
  확인이 필요한 오류로 남깁니다. 전체 이미지 디코딩·ZIP 전수 검사는 추가하지 않았습니다.

기존 `retry --job ID`, `rescan --job ID --mode full`과 GUI 작품 재검사에 공통 적용됩니다.
오프라인 회귀 검증: `node --test tests/downloader_image_resume.test.js`.
